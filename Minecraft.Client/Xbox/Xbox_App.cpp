...
#include "../Common/XUI/XUI_HelpHowToPlay.h"	#include "../Common/XUI/XUI_HowToPlayMenu.h"
...
#include "../Common/XUI/XUI_DebugSetCamera.h"//#include "..\Common\XUI\XUI_TransferToXboxOne.h"
...
int CConsoleMinecraftApp::Callback_TMSPPReadDLCFile(void *pParam,int iPad, int iUserData, C4JStorage::PTMSPP_FILEDATA pFileData,LPCSTR szFilename)
{
	...
	{
		// the DLC.xml file is now compressed (or uncompressed when compSize==0, e.g. from localhost store server)
		...
		if(uiCompSize==0 && pFileData->dwSize>=8u+uiDecompSize)
		{
			// Uncompressed: raw XML at offset 8 (localhost store recreation)
			memcpy(pDecompressedData,&((unsigned int *)pFileData->pbData)[2],uiDecompSize);
		}
		else
		{
