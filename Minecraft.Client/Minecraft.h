...
public:
	...
	// WIDESCREEN FIX: returns the true physical aspect ratio for use in
	// perspective projection. Keep vertical FOV constant and derive horizontal
	// FOV from this value so wider screens show more of the world (hor+).
	float getAspectRatio() const
	{
		if( height_phys > 0 )
			return (float)width_phys / (float)height_phys;
		return 16.0f / 9.0f; // safe fallback
	}
...
public:
	...
	void onWindowResize(int width, int height) { resize(width, height); }
private:
