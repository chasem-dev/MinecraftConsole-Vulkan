...
void GameRenderer::moveCameraToPlayer(float a)
{
	...
	else if (localplayer->ThirdPersonView())
	{
		...
		else
		{
			...
			if( localplayer->ThirdPersonView() == 2 )
			{
				...
				yRot += 180.0f;
				xRot = -xRot;  // negate pitch so collision matches the flipped camera direction
			}
	...
	if (!mc->options->fixedCamera)
	{
		float interpXRot = player->xRotO + (player->xRot - player->xRotO) * a;
		float interpYRot = player->yRotO + (player->yRot - player->yRotO) * a;

		if( localplayer->ThirdPersonView() == 2 )
		{
			glRotatef(-interpXRot, 1, 0, 0);  // negate pitch for flipped view
			glRotatef(interpYRot, 0, 1, 0);
		}
		else
		{
			glRotatef(interpXRot, 1, 0, 0);
			glRotatef(interpYRot + 180, 0, 1, 0);
		}
	}
