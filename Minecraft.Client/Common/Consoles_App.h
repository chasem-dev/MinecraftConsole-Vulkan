...
public:
	...
	static unordered_map<wstring, DLC_INFO*> DLCInfo_W64;
...
DLC_INFO* GetDLCInfoByProductID(const wchar_t* pwchProductID);
#endif
...
#if _WINDOWS64
	static void RegisterW64DLC(eDLCContentType eType,
                            const wchar_t*  pwchProductID,
                            const wchar_t*  pwchBannerTextureName,
                            int             iConfig,
                            unsigned int    uiSortIndex,
                            PBYTE           pbBannerData  = nullptr,
                            DWORD           dwBannerBytes = 0);
#endif
